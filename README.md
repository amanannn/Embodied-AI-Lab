...
| 方向 | 核心问题 | 典型 Level 1 入口 | 典型 Level 2 强化 |
|---|---|---|---|
| 感知 | 机器人如何理解世界？ | 卡尔曼状态估计 | C++ Kalman 与点云处理 |
| SLAM 与导航 | 机器人如何知道自己在哪、怎么走？ | MCL、EKF-SLAM、路径规划 | SLAM 前端与 ROS2 导航集成 |
| 运动控制 | 机器人如何稳定、准确地运动？ | PID 与轨迹优化 | 实时控制循环 |
| 强化学习与模仿学习 | 机器人如何通过奖励或演示习得行为？ | Q-learning、深度强化学习、模仿学习 | 部署与策略接口 |
| 世界模型 | 机器人能否预测未来并用来决策？ | 动力学预测与 rollout | 与规划耦合的预测模型 |
| 视觉语言导航 | 机器人能否理解语言并找到语义目标？ | toy 环境语义搜索 | 更强的语义环境集成 |
| 机器人操作 | 机器人如何与物体交互？ | 运动学、动力学、抓取 | 执行接口与操作栈 |
| 大模型与机器人 | 大模型如何帮助机器人规划和行动？ | 任务分解与工具调用 | grounded 执行接口 |
| 仿真与 Sim-to-Real | 如何在仿真中训练并迁移到真实世界？ | 轻量仿真基础 | 更丰富的仿真器桥接 |
| 垂直行业应用 | 如何把这些能力落到真实场景？ | 场景化练习 | 集成式 capstone |
...
- **Level 1: Python**  
  面向本科生和入门学习者，强调从零实现、可视化、快速反馈和直觉建立。
- **Level 2: C++ or Mixed**  
  面向工程强化，强调性能、ROS2、几何、系统约束和实时性。
- **Level 3: Research**  
  面向研究扩展，连接课程版本与课题、论文和进一步实现。
...
当前建议先从 Python 基线建立直觉，后续再从方向页中进入更强的 C++ / Mixed 桥接实验。
