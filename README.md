...
- **Level 1: Core Python Lab**  
  当前主产品。纯 Python 实现，无 ROS2 / Gazebo / Isaac / GPU 依赖，可在 Python 3.10+ 运行环境中直接运行。面向本科生和入门学习者，强调从零实现、可视化、快速反馈和直觉建立。
...
| 09. 仿真与 Sim-to-Real | 提供训练环境与迁移能力基础。 | 当前方向与 `archive/legacy-experiments/04-robot-sim` 的未来迁移关系最紧密。 |
...
下面展示的是仓库内 Level 1 实验已经跑通并固化到 `figure/` 的代表性结果。运行产物仍保存在各实验的 `output/` 目录中，首页只引用精选后的公开展示资产。

| 感知：视觉闭环 | 导航：A* 迷宫路径规划 | 导航：Dijkstra 搜索展开 |
|---|---|---|
...
| 将 marker 位姿估计、光流和前景分割组合成一个可运行的视觉实验面板。 | 在已知栅格地图中展示启发式搜索的展开区域和最终最短路径。 | 与 A* 使用相同地图和最短路径，但展开区域更广，适合观察启发式搜索的价值。 |
...
> 设备策略：Level 1 面向 Python 3.10+ 运行环境，依赖以各实验 requirements.txt 为准，可使用 venv 或 Conda 管理环境；Level 2 面向 Ubuntu 开发环境，需要 ROS2 和 C++ 工具链。
